...
function renderLiveCKEditorField(Closure $configureForm): string
{
    $component = new class extends Component implements HasForms
    {
        use InteractsWithForms;

        public static Closure $configureForm;

        public ?string $content = null;

        public function form(Form $form): Form
        {
            return (static::$configureForm)($form)->statePath('data');
        }
    ...
    $component::$configureForm = $configureForm;
...
it('binds state without the live flag by default', function () {
    $html = renderLiveCKEditorField(
        fn (Form $form) => $form->schema([CKEditor::make('content')]),
    );
...
it('sets the live flag on the entangle call for a live field', function () {
    $html = renderLiveCKEditorField(
        fn (Form $form) => $form->schema([CKEditor::make('content')->live()]),
    );
...
it('asks the component to commit on blur for a field made live on blur', function () {
    $html = renderLiveCKEditorField(
        fn (Form $form) => $form->schema([CKEditor::make('content')->live(onBlur: true)]),
    );
...
it('treats lazy the same as live on blur', function () {
    $html = renderLiveCKEditorField(
        fn (Form $form) => $form->schema([CKEditor::make('content')->lazy()]),
    );
...
it('passes an explicit debounce through as milliseconds', function () {
    $html = renderLiveCKEditorField(
        fn (Form $form) => $form->schema([CKEditor::make('content')->debounce(750)]),
    );
...
it('normalises a debounce written with a unit', function () {
    $html = renderLiveCKEditorField(
        fn (Form $form) => $form->schema([CKEditor::make('content')->debounce('2s')]),
    );
...
it('lets a field override an inherited debounce with live on blur', function () {
    $html = renderLiveCKEditorField(
        fn (Form $form) => $form
            ->schema([CKEditor::make('content')->lazy()])
            ->debounce(400),
    );
